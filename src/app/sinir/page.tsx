...
export default async function SinirPage() {
  ...
  async function handleEmitirMTR(formData: FormData) {
    ...
    const geradorId = getValidString(formData.get('geradorId'));
    const transportadorId = getValidString(formData.get('transportadorId'));
    const destinadorId = getValidString(formData.get('destinadorId'));

    if (!geradorId || !transportadorId || !destinadorId) {
      console.error('Invalid MTR parameters');
      return;
    }
